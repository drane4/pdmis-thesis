...
if(ISSET($_POST['submit'])){
   ...
   $q1 = $conn->query ("SELECT * FROM `nephrologist` WHERE BINARY `nephrologistid` = '$nephrologistid'") or die(mysqli_error());
    ...
    if($check > 0){
        ...
        $conn->query ("INSERT INTO `nephrologist` VALUES('', '$id', '$n_lname', '$n_fname', '$n_mname', '$telephone', '$mobile', '$address')") or die(mysqli_error());
...
echo "<script>document.location='../nephrologist.php'</script>"
?>